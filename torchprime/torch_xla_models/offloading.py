...
def remat_all_and_offload_these_inputs(
  ...
  *,
  ...
  bw_name_in_input_names = get_name_in_input_names(
    bwd, num_fwd_outputs, fw_name_in_output_indices
  )

  for name in names_to_offload_set:
    assert name in fw_name_in_output_indices, f"""In the forward graph:
{fwd.print_readable()}

In the backward graph:
{bwd.print_readable()}

Did not find {name} in fw_name_in_output_indices: {fw_name_in_output_indices}.
"""
    assert name in bw_name_in_input_names, f"""In the forward graph:
{fwd.print_readable()}

In the backward graph:
{bwd.print_readable()}

Did not find {name} in bw_name_in_input_names: {bw_name_in_input_names}.
"""
