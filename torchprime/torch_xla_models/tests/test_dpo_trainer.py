"""Tests for the :class:`DPOTrainer` class."""
...
from torchprime.torch_xla_models.trainer.dpo_trainer import DPOTrainer
...
class DummyDataset(Dataset):
  ...
  def __getitem__(self, idx):
    return {
      "chosen_input_ids": torch.ones(4, device=self.device),
      "chosen_attention_mask": torch.ones(4, device=self.device),
      "chosen_labels": torch.ones(4, dtype=torch.long, device=self.device),
      "rejected_input_ids": torch.zeros(4, device=self.device),
      "rejected_attention_mask": torch.ones(4, device=self.device),
      "rejected_labels": torch.zeros(4, dtype=torch.long, device=self.device),
    }
...
def test_dpo_trainer(monkeypatch, dummy_config):
  ...
  trainer = DPOTrainer(model, dummy_config, dataset)
