...
class Trainer:
  ...
  def _checkpoint_model(self, model: nn.Module):
    ...
    if len(offload_tensors):
      logger.info(
        f"Will offload these tensors to host RAM when not needed: {offload_tensors}"
      )
      if layers_to_scan is None:
        raise NotImplementedError("Host offloading only works with scan")
      if len(classes) != 1:
        raise NotImplementedError(
          "Host offloading only works when activation checkpointing exactly one layer"
        )
    if layers_to_scan is not None:
      ...
      if classes:
        # When scan is requested, we implement activation checkpointing via a
        # graph partitioner instead of `checkpoint_module`.
