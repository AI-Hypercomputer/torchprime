...
class TransAct(nn.Module):
    ...
    def get_sample_inputs(self, batch_size: int) -> tuple[tuple[Any, ...], dict[str, Any]]:
      ...
      i.e.
        args, kwargs = self.get_sample_inputs(1)
        res = self.foward(*args, **kwargs) 
      works
      """
      full_seq_len = 100
      action_type_seq = torch.randint(0, 20, (batch_size, full_seq_len))
      item_embedding_seq = torch.rand(batch_size, full_seq_len, self.item_emb_dim)
      action_time_seq = torch.randint(0, 20, (batch_size, full_seq_len))
      request_time = torch.randint(500, 1000, (batch_size,))
      item_embedding = torch.rand(batch_size, self.item_emb_dim)
      input_features = (
          action_type_seq,
          item_embedding_seq,
          action_time_seq,
