...
class MFU:
  ...
  """The hardware floating point throughput during one training step, in teraflops."""
...
# TODO(yifeit): can we share the impl with MaxText.
...
def compute_mfu(
  ...
  Args:
    config: a dictionary representing a decoded JSON HuggingFace model config.
    batch_size: global batch size.
    sequence_length: number of tokens in each training example.
    step_duration: duration of one training step.
  ...
  """
  total_tflops, _learnable_weight_tflops, _attention_tflops = (
  ...
  match version:
    ...
    case "v5e":
      ...
      tflops_per_chip = 197
    case "v6e":
      # See https://docs.google.com/document/d/1oJYtQ-l94KWqEw9eLnZmpJImmoz5CiXZnXMBUvYmLyI/edit?tab=t.0#heading=h.vbu1oij85icr.
      # NOTE: Private number. Do not publicize.
      chip_count = core_count
      tflops_per_chip = 918
    case _:
  ...
  return MFU(
    ...
    per_device_tflops_per_sec=total_tflops / step_duration / chip_count,
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="Calculate MFU.")
  parser.add_argument(
    "--config",
    type=str,
    required=True,
    help="Model config directory name (e.g. train-llama2-7b)",
  )
  ...
  v5GlobalBatchSize = args.batch_size
  v5SeqLen = 4096
  v5StepTime = args.step_duration
  mfu = compute_mfu(
    config=json.loads((Path(args.config) / "config.json").read_text()),
    batch_size=v5GlobalBatchSize,
    sequence_length=v5SeqLen,
    step_duration=v5StepTime,
  ...
  print(
    f"Per-device realized teraflops per sec: {mfu.per_device_tflops_per_sec}",
    file=sys.stderr,
  )
  print(mfu.mfu)
